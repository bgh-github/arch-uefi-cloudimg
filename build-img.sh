...
cat > "/boot/loader/entries/$(grep --perl-regexp --only-matching '^ID=\K.*' /etc/os-release | tr --delete '"').conf" << EOF
title $(grep --perl-regexp --only-matching '^PRETTY_NAME=\K.*' /etc/os-release | tr --delete '"')
linux /vmlinuz-linux
